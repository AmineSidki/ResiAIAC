...
Chaque module métier du backend (une entité et ses opérations associées) suit rigoureusement la même organisation en couches, illustrée par la Figure~\ref{fig:architecture-couches}. Le Controller REST ne dépend que de l'\textbf{interface} du service, jamais de son implémentation concrète : ce découplage permet de faire évoluer ou remplacer la logique métier d'un service sans modifier son point d'entrée, et facilite l'écriture de tests isolés à chaque couche (voir \S1.2.8).
